...
// =============================================
// ZEWNĘTRZNY DASHBOARD (LANGER STYLE)
// =============================================
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', sans-serif; background: var(--deep-blue); min-height: 100vh; color: var(--white); }
        #dashboard { max-width: 1100px; margin: 0 auto; padding: 30px 20px; }
        .header { text-align: center; margin-bottom: 40px; }
        .header .year-badge { display: inline-block; background: var(--flame); color: var(--white); padding: 12px 30px; border-radius: 4px; font-weight: 700; font-size: 0.9em; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 15px; }
        .header .date-range { color: var(--ash-grey); font-size: 0.95em; }
        .header .current-date { font-size: 2.5em; font-weight: 700; margin-top: 15px; }
        .header .day-number { color: var(--flame); font-size: 1.2em; font-weight: 600; margin-top: 8px; }
        .tabs-container { display: flex; justify-content: center; margin-bottom: 30px; gap: 5px; background: rgba(255,255,255,0.05); padding: 5px; border-radius: 8px; width: fit-content; margin-left: auto; margin-right: auto; }
        .tab-btn { padding: 14px 35px; font-size: 0.95em; font-weight: 600; border: none; border-radius: 6px; cursor: pointer; transition: all 0.3s; background: transparent; color: var(--ash-grey); font-family: inherit; }
        .tab-btn:hover { color: var(--white); }
        .tab-btn.active { background: var(--flame); color: var(--white); }
        .tab-content { display: none; }
        .tab-content.active { display: block; }
        .day-selector-container { text-align: center; margin-bottom: 30px; }
        .day-selector-container label { display: block; font-weight: 500; margin-bottom: 10px; color: var(--ash-grey); font-size: 0.9em; text-transform: uppercase; }
        .day-selector { padding: 14px 25px; font-size: 16px; border: 2px solid rgba(255,255,255,0.1); border-radius: 8px; cursor: pointer; width: 100%; max-width: 280px; background: rgba(255,255,255,0.05); color: var(--white); font-family: inherit; }
        .day-selector:focus { outline: none; border-color: var(--flame); }
        .day-selector option { background: var(--deep-blue); color: var(--white); }
        ...
        .card h3 { color: var(--white); font-weight: 600; margin-bottom: 20px; font-size: 1em; text-transform: uppercase; display: flex; align-items: center; gap: 10px; }
        .card h3::before { content: ''; width: 4px; height: 20px; background: var(--flame); border-radius: 2px; }
        #habits-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin-bottom: 25px; }
        .habit-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 25px 20px; text-align: center; transition: all 0.3s; }
        .habit-card:hover { transform: translateY(-3px); }
        .habit-card .icon { font-size: 2.2em; margin-bottom: 12px; }
        .habit-card .name { font-size: 0.85em; color: var(--ash-grey); margin-bottom: 10px; font-weight: 500; text-transform: uppercase; }
        .habit-card .status { font-size: 1.8em; font-weight: 700; }
        .habit-card.done { background: var(--flame); border-color: var(--flame); }
        .habit-card.done .name { color: rgba(255,255,255,0.9); }
        .habit-card.done .status { color: var(--white); }
        .habit-card.not-done .status { color: rgba(255,255,255,0.2); }
        .habit-card.steps { background: var(--cobalt); border-color: var(--cobalt); }
        .habit-card.steps .name, .habit-card.steps .status { color: var(--white); }
        .habit-card.steps .status { font-size: 1.3em; }
        .habit-card.sport { background: linear-gradient(135deg, var(--flame) 0%, #ff6b3d 100%); }
        .habit-card.sport .name, .habit-card.sport .status { color: var(--white); }
        .habit-card.sport .status { font-size: 0.95em; }
        .streaks-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 15px; margin-bottom: 25px; }
        .streak-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 20px 15px; text-align: center; position: relative; }
        .streak-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: var(--flame); }
        .streak-card .icon { font-size: 1.8em; margin-bottom: 8px; }
        .streak-card .value { font-size: 2em; font-weight: 700; color: var(--flame); display: flex; align-items: center; justify-content: center; gap: 5px; }
        .streak-card .label { font-size: 0.75em; color: var(--ash-grey); text-transform: uppercase; margin-top: 5px; }
        .calorie-balance-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 25px; margin-bottom: 25px; }
        .calorie-balance-card h3 { color: var(--white); font-weight: 600; margin-bottom: 20px; font-size: 1em; text-transform: uppercase; display: flex; align-items: center; gap: 10px; }
        .calorie-balance-card h3::before { content: ''; width: 4px; height: 20px; background: var(--flame); border-radius: 2px; }
        .calorie-stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 20px; }
        .calorie-stat-box { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; padding: 20px; text-align: center; }
        .calorie-stat-box .value { font-size: 2em; font-weight: 700; }
        .calorie-stat-box .label { font-size: 0.8em; color: var(--ash-grey); text-transform: uppercase; margin-top: 5px; }
        .calorie-stat-box.deficit .value { color: var(--green); }
        .calorie-stat-box.surplus .value { color: var(--red); }
        .calorie-stat-box.under .value { color: var(--green); }
        .calorie-stat-box.over .value { color: var(--yellow); }
        .total-balance { margin-top: 20px; padding: 20px; border-radius: 10px; text-align: center; }
        .total-balance.deficit { background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); }
        .total-balance.surplus { background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); }
        .total-balance .big-value { font-size: 2.5em; font-weight: 700; }
        .total-balance.deficit .big-value { color: var(--green); }
        .total-balance.surplus .big-value { color: var(--red); }
        .total-balance .big-label { font-size: 0.9em; color: var(--ash-grey); margin-top: 5px; }
        #macro-chart-area { display: flex; justify-content: center; align-items: center; position: relative; max-width: 260px; height: 260px; margin: 0 auto 25px; }
        #macro-center-text { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; }
        #macro-center-text .total-kcal { font-size: 2em; font-weight: 700; color: var(--white); }
        #macro-center-text .label { font-size: 0.75em; color: var(--ash-grey); text-transform: uppercase; }
        #daily-macro-summary { display: flex; justify-content: center; gap: 30px; flex-wrap: wrap; }
        .macro-item { text-align: center; }
        .macro-item .value { font-size: 1.5em; font-weight: 700; display: block; margin-bottom: 4px; }
        .macro-item .label { font-size: 0.8em; color: var(--ash-grey); text-transform: uppercase; }
        ...
        .data-table th { background: var(--flame); color: var(--white); padding: 14px 12px; text-align: center; font-weight: 600; font-size: 0.85em; text-transform: uppercase; }
        .data-table td { padding: 14px 12px; text-align: center; border-bottom: 1px solid rgba(255,255,255,0.05); color: var(--ash-grey); }
        .data-table tbody tr:hover { background: rgba(255,255,255,0.03); }
        .chart-container { position: relative; height: 250px; }
        .loading-message { text-align: center; padding: 60px; color: var(--ash-grey); }
        .loading-message .spinner { width: 40px; height: 40px; border: 3px solid rgba(255,255,255,0.1); border-top-color: var(--flame); border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 20px; }
        @keyframes spin { to { transform: rotate(360deg); } }
        @media (max-width: 600px) {
            .header .current-date { font-size: 1.8em; }
            .tab-btn { padding: 12px 20px; font-size: 0.9em; }
            .streaks-grid, .calorie-stats-grid { grid-template-columns: repeat(2, 1fr); }
        }
    </style>
</head>
<body>
    <div id="dashboard">
        <header class="header">
            ...
            <div class="date-range" id="date-range">Ładowanie...</div>
        ...
        <div id="main-content" style="display: none;">
            ...
            <div id="habits-tab" class="tab-content active">
                <div class="day-selector-container">
                    <label for="habits-day-selector">Wybierz dzień</label>
            ...
            <div id="diet-tab" class="tab-content">
                <div class="day-selector-container">
                    <label for="diet-day-selector">Wybierz dzień</label>
                ...
                <div class="card">
                    <h3>Historia makroskładników</h3>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        let stepsChartInstance = null, macroChartInstance = null, caloriesChartInstance = null;
        ...
        fetch(`${AJAX_URL}?action=habits_dashboard_data&date_start=${dateStart}&date_end=${dateEnd}`)
            .then(r => r.json())
            .then(result => {
                if (!result.success) throw new Error(result.data || 'Błąd');
            ...
            .catch(error => {
                loadingMessage.innerHTML = `<p style='color: var(--flame);'>Nie udało się załadować danych.<br>Błąd: ${error.message}</p>`;
            });
        ...
        function updateHeader() {
            ...
            document.getElementById('current-date-display').textContent = `${dniTygodnia[today.getDay()]}, ${today.getDate()} ${miesiace[today.getMonth()]} ${today.getFullYear()}`;
            if (dashboardData.period) document.getElementById('date-range').textContent = `Okres: ${dashboardData.period.nazwa}`;
        ...
        function getUniqueDatesFromData() {
            const dates = new Set();
            if (dashboardData.entries) dashboardData.entries.forEach(e => dates.add(e.dzien));
            if (dashboardData.sport) dashboardData.sport.forEach(s => dates.add(s.dzien));
            if (dashboardData.diet) dashboardData.diet.forEach(d => dates.add(d.dzien));
        ...
        function initializeHabitsTab() {
            ...
            uniqueDates.forEach(date => {
                ...
                option.value = date;
                option.textContent = new Date(date).toLocaleDateString('pl-PL', { day: 'numeric', month: 'short', year: 'numeric' });
                selector.appendChild(option);
            });
            selector.addEventListener('change', () => updateHabitsView(selector.value));
            if (uniqueDates.length > 0) updateHabitsView(uniqueDates[0]);
            updateStreaks();
            updateSportStats();
            createStepsChart();
        }
        ...
        function updateHabitsView(dateISO) {
            ...
            const dayEntries = dashboardData.entries ? dashboardData.entries.filter(e => e.dzien === dateISO) : [];
            const daySport = dashboardData.sport ? dashboardData.sport.filter(s => s.dzien === dateISO) : [];
            let html = '';
            if (dashboardData.habits) {
                dashboardData.habits.forEach(habit => {
                    ...
                    html += `<div class="habit-card ${isDone ? 'done' : 'not-done'}"><div class="icon">${habit.ikona || '📋'}</div><div class="name">${habit.nazwa}</div><div class="status">${minutes > 0 ? minutes + ' min' : (isDone ? '✓' : '○')}</div></div>`;
                });
            }
            const activities = daySport.filter(s => s.typ !== 'kroki');
            if (activities.length > 0) {
                html += `<div class="habit-card sport"><div class="icon">🏋️</div><div class="name">Sport</div><div class="status">${activities.map(a => a.typ).join(', ')}</div></div>`;
            }
            const stepsEntry = daySport.find(s => s.typ === 'kroki');
            const steps = stepsEntry ? parseInt(stepsEntry.kroki) : 0;
            html += `<div class="habit-card ${steps > 0 ? 'steps' : 'not-done'}"><div class="icon">👟</div><div class="name">Kroki</div><div class="status">${steps.toLocaleString('pl-PL')}</div></div>`;
            grid.innerHTML = html;
        }

        function updateStreaks() {
            const container = document.getElementById('all-streaks-grid');
            if (!dashboardData.stats) { container.innerHTML = '<p style="color: var(--ash-grey);">Brak danych</p>'; return; }
            ...
            container.innerHTML = `
                <div class="streak-card"><div class="icon">🔥</div><div class="value">${currentStreak} 🔥</div><div class="label">Aktualny streak</div></div>
                <div class="streak-card"><div class="icon">🏆</div><div class="value">${bestStreak}</div><div class="label">Najlepszy streak</div></div>
                <div class="streak-card"><div class="icon">⭐</div><div class="value">${totalXp}</div><div class="label">Całkowite XP</div></div>
                <div class="streak-card"><div class="icon">📈</div><div class="value">${level}</div><div class="label">Poziom</div></div>`;
        }

        function updateSportStats() {
            const container = document.getElementById('sport-stats');
            if (!dashboardData.sport || dashboardData.sport.length === 0) { container.innerHTML = '<p style="color: var(--ash-grey); text-align: center;">Brak danych sportowych</p>'; return; }
            let totalSteps = 0, daysWithSteps = 0;
            const activityCount = {}, stepsByDay = {};
            dashboardData.sport.forEach(s => {
                if (s.typ === 'kroki' && s.kroki > 0) stepsByDay[s.dzien] = (stepsByDay[s.dzien] || 0) + parseInt(s.kroki);
                else if (s.typ !== 'kroki') activityCount[s.typ] = (activityCount[s.typ] || 0) + 1;
            });
            Object.values(stepsByDay).forEach(steps => { totalSteps += steps; daysWithSteps++; });
            const avgSteps = daysWithSteps > 0 ? Math.round(totalSteps / daysWithSteps) : 0;
            const typeIcons = { 'silownia': '🏋️', 'bieganie': '🏃', 'rower': '🚴', 'plywanie': '🏊', 'padel': '🎾', 'spacer': '🚶', 'inne': '⚡' };
            let html = `<div class="streaks-grid"><div class="streak-card"><div class="icon">👟</div><div class="value">${totalSteps.toLocaleString('pl-PL')}</div><div class="label">Kroków łącznie</div></div><div class="streak-card"><div class="icon">📊</div><div class="value">${avgSteps.toLocaleString('pl-PL')}</div><div class="label">Średnia/dzień</div></div>`;
            Object.entries(activityCount).forEach(([typ, count]) => { html += `<div class="streak-card"><div class="icon">${typeIcons[typ] || '⚡'}</div><div class="value">${count}x</div><div class="label">${typ}</div></div>`; });
            container.innerHTML = html + '</div>';
        }

        function createStepsChart() {
            const stepsByDay = {};
            if (dashboardData.sport) dashboardData.sport.forEach(s => { if (s.typ === 'kroki' && s.kroki > 0) stepsByDay[s.dzien] = parseInt(s.kroki); });
            const labels = [], data = [];
            for (let i = 13; i >= 0; i--) {
            ...
            stepsChartInstance = new Chart(ctx, { type: 'bar', data: { labels, datasets: [{ label: 'Kroki', data, backgroundColor: '#0026ff', borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#c4c4c4' } }, x: { grid: { display: false }, ticks: { color: '#c4c4c4' } } } } });
        }

        function initializeDietTab() {
            const selector = document.getElementById('diet-day-selector');
            if (!dashboardData.diet || dashboardData.diet.length === 0) { document.getElementById('calorie-stats-grid').innerHTML = '<p style="color: var(--ash-grey);">Brak danych diety</p>'; return; }
            dashboardData.diet.forEach(d => {
                const option = document.createElement('option');
                option.value = d.dzien;
                option.textContent = new Date(d.dzien).toLocaleDateString('pl-PL', { day: 'numeric', month: 'short', year: 'numeric' });
                selector.appendChild(option);
            });
            selector.addEventListener('change', () => updateDietView(selector.value));
            if (dashboardData.diet.length > 0) updateDietView(dashboardData.diet[0].dzien);
        ...
        function updateDietView(dateISO) {
            ...
            if (!dayData) return;
            const kcal = parseFloat(dayData.kalorie) || 0, p = parseFloat(dayData.bialko) || 0, f = parseFloat(dayData.tluszcze) || 0, c = parseFloat(dayData.weglowodany) || 0;
            ...
            if (macroChartInstance) { macroChartInstance.data.datasets[0].data = macroData; macroChartInstance.update(); }
            else {
                macroChartInstance = new Chart(document.getElementById('macroChart').getContext('2d'), { type: 'doughnut', data: { labels: ['Białko', 'Tłuszcze', 'Węglowodany'], datasets: [{ data: macroData, backgroundColor: ['#4CAF50', '#FFC107', '#0026ff'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '75%', plugins: { legend: { position: 'bottom', labels: { color: '#c4c4c4', padding: 15, usePointStyle: true } } } } });
            }
            document.getElementById("daily-macro-summary").innerHTML = `<div class="macro-item protein"><span class="value">${p.toFixed(1)}g</span><span class="label">Białko</span></div><div class="macro-item fat"><span class="value">${f.toFixed(1)}g</span><span class="label">Tłuszcze</span></div><div class="macro-item carbs"><span class="value">${c.toFixed(1)}g</span><span class="label">Węglowodany</span></div>`;
        }

        function updateCalorieBalance() {
            if (!dashboardData.diet || dashboardData.diet.length === 0) return;
            let daysUnder = 0, daysOver = 0, totalDeficit = 0, totalSurplus = 0;
            dashboardData.diet.forEach(d => { const diff = (parseFloat(d.kalorie) || 0) - CALORIE_GOAL; if (diff < 0) { daysUnder++; totalDeficit += Math.abs(diff); } else if (diff > 0) { daysOver++; totalSurplus += diff; } });
            const netBalance = totalSurplus - totalDeficit, isDeficit = netBalance < 0;
            document.getElementById("calorie-stats-grid").innerHTML = `<div class="calorie-stat-box under"><div class="value">${daysUnder}</div><div class="label">Dni poniżej ${CALORIE_GOAL}</div></div><div class="calorie-stat-box over"><div class="value">${daysOver}</div><div class="label">Dni powyżej ${CALORIE_GOAL}</div></div><div class="calorie-stat-box deficit"><div class="value">${Math.round(totalDeficit).toLocaleString('pl-PL')}</div><div class="label">Suma deficytu (kcal)</div></div><div class="calorie-stat-box surplus"><div class="value">${Math.round(totalSurplus).toLocaleString('pl-PL')}</div><div class="label">Suma nadwyżki (kcal)</div></div>`;
            document.getElementById("total-balance-container").innerHTML = `<div class="total-balance ${isDeficit ? 'deficit' : 'surplus'}"><div class="big-value">${isDeficit ? '-' : '+'}${Math.abs(Math.round(netBalance)).toLocaleString('pl-PL')} kcal</div><div class="big-label">${isDeficit ? 'Łączny deficyt kaloryczny' : 'Łączna nadwyżka kaloryczna'}</div></div>`;
        }

        function createCaloriesChart() {
            if (!dashboardData.diet || dashboardData.diet.length === 0) return;
            const sortedDiet = [...dashboardData.diet].sort((a, b) => new Date(a.dzien) - new Date(b.dzien)).slice(-14);
            const labels = sortedDiet.map(d => { const date = new Date(d.dzien); return `${date.getDate()}/${date.getMonth() + 1}`; });
            const data = sortedDiet.map(d => parseFloat(d.kalorie) || 0);
            const ctx = document.getElementById('caloriesChart').getContext('2d');
            if (caloriesChartInstance) caloriesChartInstance.destroy();
            caloriesChartInstance = new Chart(ctx, { type: 'line', data: { labels, datasets: [{ label: 'Kalorie', data, borderColor: '#ff4d1c', backgroundColor: 'rgba(255, 77, 28, 0.1)', fill: true, tension: 0.3, pointBackgroundColor: '#ff4d1c', pointRadius: 5, borderWidth: 3 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: false, grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#c4c4c4' } }, x: { grid: { display: false }, ticks: { color: '#c4c4c4' } } } } });
        }

        function createDietTable() {
            if (!dashboardData.diet || dashboardData.diet.length === 0) { document.getElementById('diet-table-wrapper').innerHTML = '<p style="color: var(--ash-grey); text-align: center;">Brak danych</p>'; return; }
            const sortedDiet = [...dashboardData.diet].sort((a, b) => new Date(a.dzien) - new Date(b.dzien));
            let html = '<table class="data-table"><thead><tr><th>Data</th><th>Kalorie</th><th>Białko</th><th>Tłuszcze</th><th>Węglowodany</th><th>Bilans</th></tr></thead><tbody>';
            sortedDiet.forEach(d => {
                const date = new Date(d.dzien), kcal = parseFloat(d.kalorie) || 0, diff = kcal - CALORIE_GOAL;
                html += `<tr><td style="color: #fff; font-weight: 500;">${date.getDate()}/${date.getMonth() + 1}</td><td>${Math.round(kcal).toLocaleString('pl-PL')} kcal</td><td>${parseFloat(d.bialko).toFixed(1)}g</td><td>${parseFloat(d.tluszcze).toFixed(1)}g</td><td>${parseFloat(d.weglowodany).toFixed(1)}g</td><td style="color: ${diff < 0 ? '#22c55e' : '#ef4444'}; font-weight: 600;">${diff < 0 ? Math.round(diff) : '+' + Math.round(diff)}</td></tr>`;
            });
            document.getElementById('diet-table-wrapper').innerHTML = html + '</tbody></table>';
        }
    });
    </script>
</body>
